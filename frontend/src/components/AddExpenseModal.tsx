...
interface Props {
  ...
  groupId: string; // reservado para el futuro si se quiere hacer un POST a la API
  members: string[];
}

function AddExpenseModal({ onClose, onSubmit, groupId, members }: Props) {
  const [step, setStep] = useState(1);
  const [description, setDescription] = useState("");
  const [creditors, setCreditors] = useState<{ name: string; amount: number }[]>([
    { name: "", amount: 0 },
  ]);
  ...
  return (
    <div className="modal-overlay">
      ...
      <button className="close-modal" onClick={onClose}>
        <img src={crossIcon} alt="close-expense-addition" width={17} height={17} style={{marginTop:"2px"}} />
      </button>
